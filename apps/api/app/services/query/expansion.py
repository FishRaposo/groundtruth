...
class QueryExpander:
    ...
    async def _llm_expand(self, query: str, num_expansions: int) -> list[str]:
        ...
        prompt = (
            ...
            " search query.\n"
            "These should capture the same intent but use different keywords,"
            ...
            "- Make each one different enough to potentially match different"
            " documents\n"
            "\n"
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are a search query expansion assistant. Generate "
                        "alternative phrasings that capture the same search "
                        "intent."
                    ),
                },
    ...
    async def generate_synonyms(self, terms: list[str]) -> dict[str, list[str]]:
        ...
        prompt = (
            "For each of the following terms, provide 3-5 synonyms or related"
            " terms:\n"
            "\n"
            f"Terms: {terms_str}\n"
            "\n"
...
class HyDENetwork:
    ...
    async def generate_hypothetical_document(self, query: str) -> str:
        ...
        prompt = (
            "Write a short passage that would answer or address the following"
            " query.\n"
            "This should be written as if it were a document from a knowledge"
            " base.\n"
            "\n"
            f"Query: {query}\n"
            "\n"

...
class OCRUnavailableError(RuntimeError):
    """Raised when OCR is requested but its optional dependencies are missing.

    This is the offline default. Callers (e.g. API routes) should translate
    this into a clean 503 rather than leaking a raw 500.
    """
...
class OCRService:
    ...
    def _document_path(document: Document) -> str:
        ...
        Mirrors the storage convention used by the ingestion/upload pipeline:
        ``data/uploads/{id}/{id}{ext}`` where the extension comes from the
        document title. The raw title is never used directly in the path.
        """
    ...
    async def process_document(
        ...
        file_path = Path(self._document_path(document))
    ...
    async def _process_pdf(
        ...
        for page_num, image in enumerate(images, start=1):
            ...
            if page_blocks:
                total_confidence += sum(b.confidence for b in page_blocks) / len(
                    page_blocks
                )

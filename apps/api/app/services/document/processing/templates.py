...
from app.services.document.processing.ocr import OCRBlock, OCResult
...
class TemplateExtractor:
    ...
    BUILT_IN_TEMPLATES: dict[str, dict[str, Any]] = {
        "invoice": {
            ...
            "fields": [
                FieldDefinition(
                    "invoice_number", "Invoice #", "text", True, r"INV?[-#]?\s*(\w+)"
                ),
                FieldDefinition(
                    "date", "Date", "date", True, r"(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})"
                ),
                FieldDefinition(
                    "total", "Total", "number", True, r"[$ۣ]?\s*([\d,]+\.?\d*)"
                ),
    ...
    def _find_by_proximity(
        ...
        for _i, block in enumerate(sorted_blocks):

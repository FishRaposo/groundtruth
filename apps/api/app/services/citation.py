...
class CitationService:
    ...
    async def assemble_citations(
        ...
        referenced_indices = {int(m) for m in matches}

        # Number citations by their position in this (reranked) list, matching
        # how generation._build_prompt numbers sources with enumerate(context, 1).
        # chunk.chunk_index is the chunk's offset within its source document and
        # is unrelated to list position, so it must not drive citation numbers.

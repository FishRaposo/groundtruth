...
class WorkflowInstance(Base):
    ...
    )  # pending, in_progress, approved, rejected, cancelled, escalated, expired
    ...
    )  # manual, upload, form, scheduled, webhook

    # Metadata and context
    metadata_: Mapped[dict[str, Any]] = mapped_column("metadata", JSON, default=dict)
    context: Mapped[dict[str, Any]] = mapped_column(
        JSON, default=dict
    )  # Extracted form data, etc.
...
class WorkflowStep(Base):
    ...
    is_parallel: Mapped[bool] = mapped_column(
        Boolean, default=False
    )  # All approvers at once vs sequential
    ...
    rejection_route: Mapped[str | None] = mapped_column(
        String(100), nullable=True
    ...
    )  # waiting, pending, approved, rejected, expired

    # Decisions recorded
    decisions: Mapped[dict[str, Any]] = mapped_column(
        JSON, default=dict
    )  # {approver_id: {action, comment, timestamp}}

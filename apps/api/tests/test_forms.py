...
def _make_extracted(name: str, value: str, confidence: float = 0.9) -> ExtractedField:
    return ExtractedField(
        name=name, value=value, confidence=confidence, source_text=value
    )
...
class TestFormFiller:
    ...
    async def test_fills_from_extracted_fields(self) -> None:
        field_defs = [
            _make_field_def("invoice_number"),
            _make_field_def("total", "number"),
        ]
        extracted = [
            _make_extracted("invoice_number", "INV-001"),
            _make_extracted("total", "1200.00"),
        ]

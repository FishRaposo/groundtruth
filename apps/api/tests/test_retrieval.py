...
@pytest.mark.asyncio
async def test_retrieve_returns_empty_for_no_documents(
    service: RetrievalService,
) -> None:
    with patch.object(
        service, "hybrid_search", new_callable=AsyncMock, return_value=[]
    ):
...
async def test_hybrid_search_combines_vector_and_keyword(
    ...
    with (
        ...
        patch(
            "app.services.retrieval.service.AsyncSessionLocal",
...
async def test_similarity_search_returns_scored_results(
    ...
    chunk_id = uuid.uuid4()
    # Return a row with id + embedding that yields ~0.85 cosine similarity
    # with [0.1]*1536
    mock_row = MagicMock()
    ...
    with patch(
        "app.services.retrieval.service.AsyncSessionLocal", return_value=mock_session
    ):
...
@pytest.mark.asyncio
async def test_keyword_search_handles_special_characters(
    service: RetrievalService,
) -> None:
    chunk_id = uuid.uuid4()
    mock_row = MagicMock()
...
async def test_retrieve_respects_top_k(service: RetrievalService) -> None:
    mock_results = [_make_chunk(uuid.uuid4(), uuid.uuid4(), i) for i in range(3)]

    with patch.object(
        service, "hybrid_search", new_callable=AsyncMock, return_value=mock_results[:3]
    ) as mock_hybrid:
...
async def test_hybrid_search_with_no_results(service: RetrievalService) -> None:
    with (
        patch("app.services.retrieval.service.embedding_service") as mock_embedding,
        patch.object(
            service, "similarity_search", new_callable=AsyncMock, return_value=[]
        ),
        patch.object(
            service, "keyword_search", new_callable=AsyncMock, return_value=[]
        ),
    ):
        mock_embedding.embed_query = AsyncMock(return_value=[0.1] * 1536)
...
@pytest.mark.asyncio
async def test_rrf_fusion_gives_higher_score_to_items_in_both_results(
    service: RetrievalService,
) -> None:
    ...
    all_chunks = [
        make_mock_chunk(shared_id, 0),
        make_mock_chunk(only_vector_id, 1),
        make_mock_chunk(only_keyword_id, 2),
    ]
    mock_result = MagicMock()

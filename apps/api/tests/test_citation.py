...
def test_validate_citations_detects_missing_references(
    service: CitationService,
) -> None:
...
@pytest.fixture
def reordered_chunks() -> list[ChunkWithScore]:
    """Reranked chunks whose chunk_index does NOT match their list position.

    The reranker reorders chunks by relevance, so the first chunk in the
    reranked list can be the 5th chunk (chunk_index=4) of its source document.
    Citation numbers must follow list position, not chunk_index.
    """
    return [
        ChunkWithScore(
            id=uuid.uuid4(),
            document_id=uuid.uuid4(),
            content="Most relevant chunk, but it is chunk #5 of its document.",
            chunk_index=4,
            metadata=None,
        ...
        ChunkWithScore(
            id=uuid.uuid4(),
            document_id=uuid.uuid4(),
            content="Second most relevant, chunk #3 of its document.",
            chunk_index=2,
            metadata=None,
...
async def test_assemble_citations_numbers_by_list_position_not_chunk_index(
    service: CitationService, reordered_chunks: list[ChunkWithScore]
) -> None:
    # The generator numbers sources [1], [2] by enumerate(context, 1) over this
    # same reranked list. Citations must use the same numbering so [n] markers
    # resolve to the right source even when chunk_index != list position.
    answer = "Per [1] and [2], the policy applies."
    citations = await service.assemble_citations(reordered_chunks, answer)

    assert len(citations) == 2
    # [1] -> first reranked chunk (chunk_index=4), [2] -> second (chunk_index=2).
    assert citations[0].citation_index == 1
    assert citations[0].chunk_id == reordered_chunks[0].id
    assert citations[1].citation_index == 2
    assert citations[1].chunk_id == reordered_chunks[1].id


async def test_assemble_citations_resolves_single_marker_by_position(
    service: CitationService, reordered_chunks: list[ChunkWithScore]
) -> None:
    # Only [1] is referenced: it must map to the first chunk in the reranked
    # list, regardless of that chunk's chunk_index (4 here).
